...
void TurretSubsystem::SetAngleAbsolute(double angle) {
	SetAngle(angle + CommandBase::drive->angle());
}

double TurretSubsystem::GetAngleAbsolute() {
	return GetAngle() - CommandBase::drive->angle();
}
...
void TurretSubsystem::AimVision() {
	...
	if(last_vision != new_vision && table->GetBoolean("Found", false)) {
		...
		last_angle = last_angle + power;
	}

	// Ensure we don't aim outside of our range
	double new_angle = last_angle < 0 ? 0 : last_angle;
	new_angle = new_angle > 180 ? 180 : new_angle;
	SetAngleAbsolute(last_angle);
}
